 ...
 * @internal
 */
final class ClassSourceManipulatorV2
{
    ...
    // TODO: WIP
    public function addTransformMethod(ClassMetadataInfo $meta)
    {
        $fields = $meta->getFieldNames();
        $class = Str::getShortClassName($meta->getName());
        $variable = Str::asLowerCamelCase($class);

        $param = (new Builder\Param($variable))
        ->setTypeHint($class);

        $method = (new Builder\Method('transform'))
            ->addParam($param)
            ->makePublic();

        foreach ($fields as $field) {
            // TODO: Check the expression builder
        }
    }
